...
class Pdfgen_BillingNoteInvlice_TP7_LAMPHUN {
  ...
  static void exportPDF_BillingNoteInvlice_TP7_LAMPHUN(
    ...
    final String? _selectedMode =
        await Man_BillingNoteInvlice_PDF.showPageFormatDialog(
    ...
    if (img1 == null || img1.toString().isEmpty) {
      ...
      try {
        netImage_QR.add(
          await networkImage(
            '${MyConstant().domain_chao}/files/$foder/payment/$img1',
          ),
        );
      } catch (e) {
        // Fallback to placeholder if QR image fails
        try {
          netImage_QR.add(
            await networkImage(
              '${MyConstant().domain_chao}/Awaitdownload/imagenot.png',
          ...
          // If even placeholder fails, skip or use empty
          netImage_QR.add(null);
        }
      }
    }
...
// ฟังก์ชันปัดเศษทศนิยม 2 ตำแหน่ง
    ...
    // หาวันที่จาก KR ก่อน ถ้าไม่มีค่อยหาจาก exp_dtype != 'KD'
    final targetRecord = _InvoiceHistoryModels.firstWhere(
      (e) => e.dtype.toString() == 'KR' && e.date != null,
      orElse: () => _InvoiceHistoryModels.firstWhere(
        (e) => e.dtype.toString() != 'KD' && e.date != null,
        orElse: () => _InvoiceHistoryModels.first, // ถ้าไม่เจอเลย
      ),
    );

    final totallist = (targetRecord != null && targetRecord.billdate != null)
        ? '${DateFormat('MMM', 'th').format(DateTime.parse(targetRecord.billdate!))} ${DateTime.parse(targetRecord.billdate!).year + 543}'
        : '-';
    ...
    if (Preview_ser.toString() == 'Folder') {
      ...
      await FlutterFileDialog.saveFile(
        params: SaveFileDialogParams(
          data: data,
          fileName: 'ใบวางบิล/ใบแจ้งหนี้${cFinn}',
        ),
      );
      // final List<int> bytes = await pdf.save();
      // final Uint8List data = Uint8List.fromList(bytes);
