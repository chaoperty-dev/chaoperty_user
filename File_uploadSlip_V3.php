...
/* =====================================================================
 *  สร้าง path ปลายทาง
 *  โครงสร้าง: files/{Foder}/slip/{YYYY}/{MM_YYYY}/
 * ===================================================================== */
$date_Y = (int)date('Y');
$date_M = (int)date('m');
$date_D = (int)date('d');
$sub = '';
if ($date_Y > 2024) {
    $mm = ($date_M < 10) ? ('0' . $date_M) : (string)$date_M;
    $sub = $date_Y . '/' . $mm . '_' . $date_Y;
}
$targetDir = $UPLOAD_ROOT . '/' . $Foder . '/slip/' . $sub;
if (!is_dir($targetDir)) {
 ...
 *  รูปแบบ: slip_{Foder}_{timestamp}_{rand}.png
 *  หรือถ้ามี name มา → ใช้ name ตามด้วย timestamp กันชน
 * ===================================================================== */
if ($name === '') {
    $name = 'slip_' . $Foder;
}
